...
const regex = /{mapSurfaceMarkers\.map\(\(marker\) => \{[\s\S]*?return \(\s*<button[\s\S]*?className=\{`tablet-marker tablet-marker-\$\{marker\.kind\} \$\{s[\s\S]*?key=\{marker\.id\}[\s\S]*?onClick=\{[\s\S]*?\}[\s\S]*?style=\{[\s\S]*?\}[\s\S]*?type="button"[\s\S]*?>[\s\S]*?<span className="tablet-marker-ping" \/>[\s\S]*?<span className="tablet-marker-core" \/>[\s\S]*?\{shouldShowLabel \? \([\s\S]*?\) : null\}[\s\S]*?<\/button>\s*\);\s*\}\)}/;
